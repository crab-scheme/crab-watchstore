...
- **main** bumps **once per applied Txn** (one Raft entry = one Txn = one main revision); **sub** increments per write op within that Txn, starting at 0. **main advances only when the applied op (or the executed Txn branch) changes ≥1 key** — a read-only Txn, a `DeleteRange` matching zero live keys, and a `LEASE-REVOKE` with no attached keys all leave main **unchanged** (etcd parity, `cw-u4a.40`); their response header carries the current (unbumped) revision. A `Put` always changes a key — it writes a new version even for an identical value — so it always bumps.
