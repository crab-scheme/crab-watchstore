; test/qp-rev-grant-multishard.scm — cw-6cq step 2: a client write to a WRITER
; group draws a GLOBAL revision from the AUTHORITY group and applies as PUT-AT,
; on the QUEPAXA engine. Clone of rev-grant-multishard.scm (raft) swapping the
; shard body/entry point. Two shard groups on one node (single-voter each =>
; always coordinator): shard "0" = the rev-authority, shard "1" = a writer.
; Run from repo root:  crabscheme run test/qp-rev-grant-multishard.scm
...
(define (db sk) (string-append "/tmp/cws-qp-ms-" run-tag "-a-s" sk))

; two groups on node a, both global-rev? = #t (rest[7]); voters '(a) => always coordinator.
(for-each
 (lambda (sk)
...
(section "both groups up (authority=0, writer=1)")
(spin (lambda () (and (eq? (role "0") 'leader) (eq? (role "1") 'leader))) "both coordinators")
(check "authority group 0 coordinator" 'leader (role "0"))
(check "writer group 1 coordinator"    'leader (role "1"))
